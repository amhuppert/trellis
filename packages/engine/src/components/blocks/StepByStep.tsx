...
export function StepByStep({ anchorId, title, steps, onOpenEntity = noop, onSeeEntityInGraph }: StepByStepProps) {
  return (
    <section
      id={anchorId}
        ...
        {steps.map((step, index) => (
          <li key={`${step.title}-${index}`} className="grid grid-cols-[32px_1fr] items-start gap-3.5">
            ...
            <div className="pt-0.5">
              <div className="mb-1 text-label font-semibold text-ink">{step.title}</div>
              <div className="font-serif text-label leading-relaxed text-ink-2">
                <InlineProse text={step.body} onOpenEntity={onOpenEntity} onSeeEntityInGraph={onSeeEntityInGraph} />
              </div>
            </div>
          </li>
        ))}
